...
def load_data_and_embeddings(data_path, phrase_ids_path, embeddings_path):
    ...
    train_data = read_dictionary_txt_with_phrase_ids(dictionary_path, os.path.join(
        phrase_ids_path, 'phrase_ids.train.txt'), labels_path)
    validation_data = read_dictionary_txt_with_phrase_ids(
        dictionary_path, os.path.join(phrase_ids_path, 'phrase_ids.dev.txt'), labels_path)
    test_data = read_dictionary_txt_with_phrase_ids(
        dictionary_path, os.path.join(phrase_ids_path, 'phrase_ids.test.txt'))
...
def load_data_and_embeddings_2channel(data_path, phrase_ids_path, embeddings_path):
    labels_path = os.path.join('sentiment_labels.txt')
    dictionary_path = os.path.join('dictionary.txt')
    train_data = read_dictionary_txt_with_phrase_ids(dictionary_path, os.path.join(
        phrase_ids_path, 'phrase_ids.train.txt'), labels_path)
    validation_data = read_dictionary_txt_with_phrase_ids(
        dictionary_path, os.path.join(phrase_ids_path, 'phrase_ids.dev.txt'), labels_path)
    test_data = read_dictionary_txt_with_phrase_ids(
        dictionary_path, os.path.join(phrase_ids_path, 'phrase_ids.test.txt'))
    vocab = build_vocab([train_data, validation_data, test_data])
    vocab, embeddings = load_embeddings_2channel(options.embeddings, vocab, cache=True)
    train_data = convert2ids(train_data, vocab)
    validation_data = convert2ids(validation_data, vocab)
    test_data = convert2ids(test_data, vocab)
    return train_data, validation_data, test_data, vocab, embeddings
...
def load_embeddings(path, vocab, cache=False, cache_path=None):
    ...
    with open(path) as f:
        for line in f:
            ...
            if word == UNK_TOKEN:
                raise ValueError(
                    'The unk token should not exist w.in embeddings.')
            if word in vocab:
                rows.append(line)
...
def load_embeddings_2channel(embeddings_name, vocab, cache=False, cache_path=None):
    if embeddings_name == 'glove':
        path = [os.path.expanduser('glove.840B.300d.txt.cache')]
    elif embeddings_name == 'google':
        path = [os.path.expanduser('GoogleNews-vectors-negative300.txt.cache')]
    else:  # use both
        path = [os.path.expanduser('glove.840B.300d.txt.cache'), os.path.expanduser('GoogleNews-vectors-negative300.txt.cache')]

    if len(path) == 1:
        path = path[0]
        print('loading embeddings from {}'.format(path))
        new_vocab, embeddings = load_embeddings(
            path, vocab, cache=False, cache_path=None)
        return new_vocab, embeddings
    else:
        currpath = path[0]
        print('load embeddings from glove.txt')
        one_vocab, one_embeddings = load_embeddings(
            currpath, vocab, cache=False, cache_path='google_glove.txt')
        path = path[1]
        print('load and merge embeddings from {}'.format(path))
        rows = []
        new_vocab = [UNK_TOKEN]
        intersection = [UNK_TOKEN]
        idx_in_one_vocab = []
        # Use cache file if it exists.
        # if os.path.exists(cache_path):
        #   path = cache_path
        ...
        with open(path) as f:
            for line in f:
                ...
                if word == UNK_TOKEN:
                    raise ValueError(
                        'The unk token should not exist w.in embeddings.')
                if word in one_vocab:
                    rows.append(line)
                    intersection.append(word)
                    idx_in_one_vocab.append(one_vocab[word])
                    # one_vocab[word]
        ...
        # turn vocab list into a dictionary
        new_vocab = {w: i for i, w in enumerate(intersection)}

        print('New vocab size: {}'.format(len(new_vocab)))

        # create embeddings matrix
        embeddings = np.zeros((len(new_vocab), 600), dtype=np.float32)
        for i, line in enumerate(rows):
            embeddings[i + 1] = one_embeddings[idx_in_one_vocab[i]
                                               ].tolist() + list(map(float, line.strip().split(' ')[1:]))
...
class BagOfWordsModel(nn.Module):
    def __init__(self, embeddings):
        ...
        self.embed = nn.Embedding(
            embeddings.shape[0], embeddings.shape[1], sparse=True)
...
class CNN_2channel(nn.Module):
    ...
    def __init__(self, embeddings, output_size, kernel_dim=100, kernel_sizes=(2,3,4), dropout=0.5):
        super(CNN_2channel, self).__init__()
        vocab_size = embeddings.shape[0]
        embedding_dim = embeddings.shape[1]
        self.embedding = nn.Embedding(embeddings.shape[0], embeddings.shape[1], sparse=True)
...
def run_validation(model, dataset, options):
    ...
    length = len(dataset)
    correct_label = {0: 0, 1: 0, 2: 0, 3: 0, 4: 0}
    label_total = {0: 1004, 1: 4101, 2: 13377, 3: 4900, 4: 1390}
    ...
    for data, labels, _ in batch_iterator(dataset, options.batch_size, forever=False):
        ...
        sp_len = data.shape[1]
        sp_count.setdefault(sp_len, 0)
        sp_correct.setdefault(sp_len, 0)
        sp_count[sp_len] += data.shape[0]

        sp_correct[sp_len] += (outp.data.max(1)[1] == labels).sum()
        res = outp.data.max(1)[1]

        for i in range(len(labels)):
            if res[i] == labels[i]:
                correct_label[labels[i]] += 1


    err = err / count
    acc = count / float(length)
    print("+++++++++++++++++++++++++++++++++++++++++++++++++++++")
    for key, value in sp_correct.items():
        # print(phrase_len_batch[key])
        length_acc = value / float(sp_count[key])
        print('Ev-Acc for phrase length {} is {}'.format(key, length_acc))
    print("++++++++++++++++++++++++++++++++++++++++++++++++++++++")
...
def run_test(model, dataset, options):
    print('Writing predictions to {}'.format(
        os.path.abspath(options.predictions)))
...
def run(options):
    #code for model 1245
    #train_data, validation_data, test_data, vocab, embeddings = load_data_and_embeddings(options.data, options.ids, options.embeddings)

    #code for 2-channel
    train_data, validation_data, test_data, vocab, embeddings = load_data_and_embeddings_2channel(options.data, options.ids, options.embeddings)
    ...
    #code for model 1245
    #model = CNNClassifier(len(word2index), 300, len(target2index))

    #code for 2 channel model
    model = CNN_2channel(embeddings, len(target2index))
    model.init_weights(embeddings)
    #if gpu:
    #    model = model.cuda()
    opt = optim.Adagrad(filter(lambda p: p.requires_grad, model.parameters()), lr=3e-4)
    ...
    for data, labels, _ in batch_iterator(train_data, options.batch_size, forever=True):
        ...
        if step % options.log_every == 0:
            print(
                'Step={} Tr-Loss={} Tr-Acc={}'.format(step, loss.data[0], acc))
        ...
        if step % options.eval_every == 0:
            ...
            if val_err < best_val_err:
                ...
                print('Checkpointing model step={} best_val_err={}.'.format(
                    step, best_val_err))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
    '--data', default=os.path.expanduser('~/stanfordSentimentTreebank'), type=str)
    parser.add_argument(
    '--embeddings', default=os.path.expanduser('../assignment0/GoogleNews-vectors-negative300.txt.cache'), type=str)
    parser.add_argument(
    '--model', default=os.path.join(mydir, 'model.ckpt'), type=str)
    #parser.add_argument(
    #'--predictions', default=os.path.join(mydir, 'predictions.txt'), type=str)
